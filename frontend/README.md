...
- **Build & Sichtprüfung:** Nach jeder Frontend-Änderung `npx kill-port 3000` (falls nötig) und `npm run build` ausführen. Anschließend ein Browserfenster mit `open http://localhost:3000` (oder dem aktuell genutzten Port) öffnen, um die Änderung live zu prüfen; danach das Staging-Frontend via `docker compose -f docker-compose-staging.yml up -d --build web_wineacadamy_staging` neu deployen und automatisch ein Browserfenster mit der aktualisierten Seite öffnen, um die sichtbare Änderung zu bestätigen.

...
- **Build & Sichtprüfung:** Nach jeder Frontend-Änderung zuerst `npx kill-port 3000` ausführen, anschließend lokal `npm run build` und danach `npm run dev` (kurzer Testlauf, z. B. `sleep 5 && kill $PID`), die Änderung unter `http://localhost:3000` prüfen; danach das Staging-Frontend via `docker compose -f docker-compose-staging.yml up -d --build web_wineacadamy_staging` neu deployen und automatisch ein Browserfenster mit der aktualisierten Seite öffnen, um die sichtbare Änderung zu bestätigen.

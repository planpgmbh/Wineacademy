...
- **Build & Sichtprüfung:** Nach jeder Frontend-Änderung zuerst lokal `npm run build`, anschließend `npm run dev` ausführen und die Änderung unter `http://localhost prüfen; danach das Staging-Frontend via `docker compose -f docker-compose-staging.yml up -d --build web_wineacadamy_staging` neu deployen und automatisch ein Browserfenster mit der aktualisierten Seite öffnen, um die sichtbare Änderung zu bestätigen.

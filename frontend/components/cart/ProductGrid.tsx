...
export default function ProductGrid({ products }: Props) {
  ...
  const handleAdd = (product: Produkt) => {
    addItem({
      type: product.istGutschein ? 'gutschein' : 'produkt',
      titel: product.titel,
      beschreibung: product.kurzbeschreibung,
      produktId: product.id,
      preisBrutto: product.preisBrutto ?? 0,
      steuerSatz: product.mitMwst === false ? 0 : product.steuerSatz ?? Number(process.env.NEXT_PUBLIC_DEFAULT_VAT ?? 19),
      menge: 1,
      participants: [],
    });
  };
          ...
          <article key={product.id} className="flex h-full flex-col overflow-hidden rounded-xl border shadow-sm">
            ...
            <div className="flex flex-1 flex-col p-4">
              ...
              <div className="mt-auto pt-4">
                <p className="text-sm font-medium text-gray-900">{formatPrice(product.preisBrutto ?? 0, product.waehrung || 'EUR')}</p>
                <button
